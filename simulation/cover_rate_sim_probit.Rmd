---
title: "Cover rate for probit model"
author: "Junting Ren"
date: "1/1/2021"
...
# thres = c(0,1.9,3,3.5) 
# Parameters for K=5 density spline
thres = c(0,0.6,0.7,0.8, 1,1.2,1.5,3,3.5)
#Alpha = c(-1,0.35,0.6)
Alpha = c(-1,2,-2.5)
# Cores
...
# initNULL=0.5;
write_dat = T
folder_path = "./saved_sim/"
dat_name = paste(folder_path, "cover_rate_probit", "_J=", J,"n=",n,"_", "cor=" ,cor, "_", Sys.Date(), ".R", sep = "")
table_name = paste(folder_path, "table_probit", "_J=", J,"n=",n,"_", "cor=" ,cor, Sys.Date(), ".csv", sep = "")
```
...
```{r}
paste("sim_probit", "J=", J, "n=", n, "_","cor=" ,cor, "_",  Sys.Date(), sep = "")
```
...
check_para = list(Alpha = Alpha, Gamma = gamma, thres = thres,
...
cover_list_list = foreach(t=1:sim_num,
  ...
  Bayesian_Semi_probit(Z,X,G,K=K,nIter,burnIn,thin, useCov=useCov,  byStep=byStep,max_Z = end,c = 0.01, d = 0.01, alpha_prior_var = 2.5, gamma_prior_var=2.5,check_para = check_para)
}
end_time <- Sys.time()
...
indx_conv = sapply(Sigma_eta_mean_list, function(x) x[[1]])<20
GMN_list = GMN_list[indx_conv]
thres_list = thres_list[indx_conv]
AMN_list = AMN_list[indx_conv]
AMN_cover = AMN_cover[indx_conv]
thres_cover = thres_cover[indx_conv]
...
d = data.frame(names = c(paste(rep("mean",3), c("intercept", "X1", "X2")),
           ...
           sd = c(covar_mean_sd, covar_median_sd,
                  covar_0.75_sd, dist_sd, sigma_b_delta_sd,mean(sigma_b_eta_sd)
                  ),
