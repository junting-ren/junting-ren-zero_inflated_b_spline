---
title: "Longitudinal Gamma for B-spline"
author: "Junting Ren"
date: "9/15/2021"
...
J = 10 # number of observation per subject
k = 0.7
gamma = c(1,1.2,-1)
alpha = c(0.3,1,-0.5)
...
sd_b_I = 1.1
sd_b_II = 1
data_file_name = paste("./data_gamma/dat_out_gamma_", "n_dat=", n_data, "_n=",n,"_" ,"J=",J, ".csv", sep = "")
if(!read_dat){
  ...
  for(t in 1:n_data){
    ...
    z = c()
    G = c()
    dat_matrix = matrix(cbind(x0,x1,x2),n*J,3)
    # gamma = c(1,1.2,-1)
    # alpha = c(0.3,1,-0.5)
    # delta_coef = c(-0.3,-0.5,-0.2)
    #alpha = c(2.4,0.95,-1.42)
    #delta_coef = c(0.6,-0.11,-0.2)
    b_I = rnorm(n, sd = sd_b_I)
    b_II = rnorm(n, sd = sd_b_II)
    temp1 = dat_matrix%*%as.vector(gamma)
    temp2 = dat_matrix %*% as.vector(alpha)
    temp3 = dat_matrix %*% as.vector(delta_coef)
    for(i in 1:n){
      p = pnorm(temp1[((i-1)*J+1):(i*J)]+b_I[i])
      indicator = rbinom(J, size = 1, prob = p)
      
      
      mu = temp2[((i-1)*J+1):(i*J)] + b_II[i]
      sigma = sqrt(exp(temp3[((i-1)*J+1):(i*J) ]))
      delta = k/sigma
      ...
      for(j in 1:J){
        if(indicator[j]==1){
          z = c(z, rggamma(1, theta[j], kappa[j], delta[j]))
        }else{
          z= c(z, 0)
        }
      }
    ...
    data_out_gamma_single = cbind(z, dat_matrix, G, t)
    colnames(data_out_gamma_single) = c("z", "x0", "x1", "x2","G","num")
...
```{r}
est_Alpha = apply(simplify2array(tail(MCMCfit1$alpha_array, last_n)), 1, mean)
est_thres =  apply(simplify2array(tail(MCMCfit1$thres_array, last_n)), 1, mean)
gamma_b_spline_compare_plot(x = c(1,0,0), alpha = alpha, 
                            delta_coef = delta_coef, k = k , est_Alpha=est_Alpha, 
                            phi.mat.model = MCMCfit1$Bden_raw,byStep = byStep, 
                            G = NULL, true_b_eta = NULL, MCMCfit = MCMCfit1, X = NULL,
                            G_indx = NULL, last_n = last_n, est_thres = est_thres)
```

```{r}
gamma_b_spline_compare_plot(x = c(1,1,0), alpha = alpha, 
                            delta_coef = delta_coef, k = k , est_Alpha=est_Alpha, 
                            phi.mat.model = MCMCfit1$Bden_raw,byStep = byStep, 
                            G = NULL, true_b_eta = NULL, MCMCfit = MCMCfit1, X = NULL,
                            G_indx = NULL, last_n = last_n, est_thres = est_thres)
...
```{r}
gamma_b_spline_compare_plot(x = c(1,0,1), alpha = alpha, 
                            delta_coef = delta_coef, k = k , est_Alpha=est_Alpha, 
                            phi.mat.model = MCMCfit1$Bden_raw,byStep = byStep, 
                            G = NULL, true_b_eta = NULL, MCMCfit = MCMCfit1, X = NULL,
                            G_indx = NULL, last_n = last_n, est_thres = est_thres)
...
dat_name = paste(folder_path, "compare_gamma", "n=",n,"_", "J=",J,"_","K_model = ",K_model ,"_", Sys.Date(), ".RData", sep = "")
table_name = paste(folder_path, "table_compare_gamma", "n=",n,"_", "J=",J,"_","K_model = ",K_model ,"_", Sys.Date(), ".csv", sep = "")
```

Simualtion html name
```{r}
paste("compare_gamma", "n=",n,"_", "K_model = ",K_model ,"_",Sys.Date(), sep = "")
```
