...
meanCI_probit <- function(model, level = 0.05, last_n = 1000, true_para = NULL,return_all = T){
  ...
  if(return_all == T){
    ...
    if(model$grouped & zero_inflate){
      # sigma_delta
      sigma_b_delta_sq_mean = mean(unlist(tail(model$sigma_b_delta_sq, last_n)))
      sigma_b_delta_sq_upper = quantile(unlist(tail(model$sigma_b_delta_sq, last_n)), 1 - level/2)
      sigma_b_delta_sq_lower = quantile(unlist(tail(model$sigma_b_delta_sq, last_n)), level/2)
      # sigma_eta
      sigma_b_eta_sq_mean = mean(unlist(tail(model$Sigma_b_eta, last_n)))
      sigma_b_eta_sq_upper = quantile(unlist(tail(model$Sigma_b_eta, last_n)), 1 - level/2)
      sigma_b_eta_sq_lower = quantile(unlist(tail(model$Sigma_b_eta, last_n)), level/2)
    ...
    # ALpha
    AMN = apply(simplify2array(tail(model$alpha_array, last_n)), 1, mean)
    AMN_upper = apply(simplify2array(tail(model$alpha_array, last_n)), 1, quantile, probs = 1 - level/2)
    AMN_lower = apply(simplify2array(tail(model$alpha_array, last_n)), 1, quantile, probs = level/2)
    Alpha_frame = data.frame(variable = covariate_names, Alpha_mean = AMN, 
                             Alpha_up = AMN_upper, Alpha_low = AMN_lower)
    
    # thres
    thres_num = length(model$thres_array[[1]])
    thres_mean = apply(simplify2array(tail(model$thres_array, last_n)), 1, mean)
    thres_upper = apply(simplify2array(tail(model$thres_array, last_n)), 1, quantile, probs = 1 - level/2)
    thres_lower = apply(simplify2array(tail(model$thres_array, last_n)), 1, quantile, probs = level/2)
    thres_frame = data.frame(variable = paste("thres", 1:thres_num), thres_mean = thres_mean, 
                             thres_up = thres_upper, thres_low = thres_lower)
    ...
    if(!is.null(true_para)){
      if(length(true_para$thres) == length(thres_frame$thres_mean)){
        thres_frame = cbind(thres_frame, true = true_para$thres)
        thres_frame$contain = thres_frame$true <= thres_frame$thres_up & 
          thres_frame$true >= thres_frame$thres_low
      }
      
      Alpha_frame = cbind(Alpha_frame, true = true_para$alpha)
    ...
    return(list(gamma_frame = Gamma_frame, alpha_frame = Alpha_frame, sigma_frame = sigma_frame,
                ...
                dist_effect_frame = dist_effect_frame, thres_frame = thres_frame))
  }else{
    mean_effect = apply(simplify2array(tail(model$effect_mean, last_n)), 1, mean)
    mean_effect_upper = apply(simplify2array(tail(model$effect_mean, last_n)), 1, quantile, probs = 1 - level/2)
    mean_effect_lower = apply(simplify2array(tail(model$effect_mean, last_n)), 1, quantile, probs = level/2)
    ...
    dist_effect_frame = data.frame(variable = c("mean","median", "perc.25", "perc.75"), 
                                   ...
                                   dist_up = dist_effect_upper, 
                                   dist_low = dist_effect_lower)
    if(!is.null(true_para$effect_mean)){
      mean_effect_frame = cbind(mean_effect_frame, true= true_para$effect_mean)
      mean_effect_frame$contain = mean_effect_frame$true <= mean_effect_frame$mean_up & 
        mean_effect_frame$true >= mean_effect_frame$mean_low
    }
    
    if(!is.null(true_para$effect_median)){
      median_effect_frame = cbind(median_effect_frame, true = true_para$effect_median)
    ...
    if(!is.null(true_para$effect_0.25)){
      perc.25_effect_frame = cbind(perc.25_effect_frame, true= true_para$effect_0.25)
      perc.25_effect_frame$contain = perc.25_effect_frame$true <= perc.25_effect_frame$perc.25_up & 
        perc.25_effect_frame$true >= perc.25_effect_frame$perc.25_low
    }
    
    if(!is.null(true_para$effect_0.75)){
      perc.75_effect_frame = cbind(perc.75_effect_frame, true = true_para$effect_0.75)
    ...
    if(!is.null(true_para$effect_dist)){
      dist_effect_frame = cbind(dist_effect_frame, true = true_para$effect_dist)
      dist_effect_frame$contain = dist_effect_frame$true <= dist_effect_frame$dist_up & 
        dist_effect_frame$true >= dist_effect_frame$dist_low
    ...
    return(list(mean_effect_frame = mean_effect_frame, median_effect_frame=median_effect_frame,
                perc.25_effect_frame=perc.25_effect_frame, perc.75_effect_frame=perc.75_effect_frame,
...
convergence_plot_probit <- function(model , save = T){
  zero_inflate = model$zero_inflate
  ...
  mcmc_Alpha = mcmc(t(sapply(model$alpha_array, cbind)))
  plot(mcmc_Alpha, ylab = "Alpha")
  mcmc_thres = mcmc(t(sapply(model$thres_array, cbind)))
  plot(mcmc_thres, ylab = "Trhes")
  ...
  if(model$grouped){
    mcmc_Sigma_eta= mcmc(unlist(model$Sigma_b_eta)) 
    plot(mcmc_Sigma_eta, ylab = "Sigma_eta_sq")
    for(i in g_indx){
      mcmc_b_eta = mcmc(sapply(model$b_eta, function(x)x[i]))
...
draw_b_spline = function(model, draw_mix = F,prior=c(0.5,0.1,0.1,0.1,0.2)){
  ...
  if(draw_mix == F){
    ...
    for(c in paste("spline", 1:n_spline, sep = "")){
      ...
      mode_g = grid[which(phi.mat[,c] == max(phi.mat[,c]))]
      local_frame = rbind(local_frame, data.frame(spline = c, start = min_g, mode = mode_g,end = max_g))
    }
    ...
    knot_df = data.frame(cbind(x = model$knots , y = rep(0, length(model$knots))))
    p = phi.mat_long %>% 
      ggplot(aes(x = grid, y = value, color = b_spline)) + 
      geom_line() + labs(y = "Density", title = "B-Spline Density", x = "Support") +
      geom_point(aes(x=x, y=y), colour="black", data = knot_df)+
...
plot_compare_2x_probit = function(x1, x2, model, last_n = 1000,
                           include_zero = F){
  # generate phi.mat
  grid=model$grid
  phi.mat=model$Bden_raw
  K_model = dim(phi.mat)[2]
  ...
  for(k in 1:K_model){
    if(k==1){
      c_vec = c(c_vec, pnorm(thres_mean[k] - x1 %*% AMN ))
    }else if(k==K){
      c_vec = c(c_vec, 1 - pnorm(thres_mean[k-1] - x1 %*% AMN ))
    }else{
      c_vec = c(c_vec, pnorm(thres_mean[k] - x1 %*% AMN) - sum(c_vec) )
    }
  }
  model_b_prior1 = c_vec
  
  c_vec = c()
  for(k in 1:K_model){
    if(k==1){
      c_vec = c(c_vec, pnorm(thres_mean[k] - x2 %*% AMN ))
    }else if(k==K){
      c_vec = c(c_vec, 1 - pnorm(thres_mean[k-1] - x2 %*% AMN ))
    }else{
      c_vec = c(c_vec, pnorm(thres_mean[k] - x2 %*% AMN) - sum(c_vec) )
    }
  }
  model_b_prior2 = c_vec
  # p0 = 1/(exp(x %*% GMN) + 1)
  ...
  CI1 = cal_CI_B_density_probit(model, x1, last_n = last_n, g = NULL)
  CI2 = cal_CI_B_density_probit(model, x2, last_n = last_n, g = NULL)
...
plot_checkfit_average_probit = function(Z, X, x = NULL, model, last_n = 1000,
                                 include_zero = F, bin = NULL){
  # generate phi.mat
  grid=model$grid
  phi.mat=model$Bden_raw
  K_model = dim(phi.mat)[2]
  ...
  for(k in 1:K_model){
    if(k==1){
      c_vec = c(c_vec, pnorm(thres_mean[k] - x %*% AMN ))
    }else if(k==K){
      c_vec = c(c_vec, 1 - pnorm(thres_mean[k-1] - x %*% AMN ))
    }else{
      c_vec = c(c_vec, pnorm(thres_mean[k] - x %*% AMN) - sum(c_vec) )
    }
  }
  ...
  if(include_zero == T){
    ...
    # getting the model density
    CI = cal_CI_B_density_probit(model, x, last_n = last_n, g = NULL)
    if(is.null(bin)){
...
plot_checkfit_individual_probit = function(Z, X, G, x = NULL, model, last_n = 1000, G_indx = NULL){
  require(gridExtra)
  # generate phi.mat
  grid=model$grid
  phi.mat=model$Bden_raw
  ...
  K_model = dim(phi.mat)[2]
  for(i in 1:length(G_indx)){
    b_eta_model = mean(sapply(tail(model$b_eta, last_n), function(x) x[G_indx[i]])) 
  ...
  plot_list = lapply(1:length(x_list), function(i){
    ...
    for(k in 1:K_model){
      if(k==1){
        c_vec = c(c_vec, pnorm(thres_mean[k] - x %*% AMN - b_eta))
      }else if(k==K){
        c_vec = c(c_vec, 1 - pnorm(thres_mean[k-1] - x %*% AMN - b_eta ))
      }else{
        c_vec = c(c_vec, pnorm(thres_mean[k] - x %*% AMN - b_eta) - sum(c_vec) )
      }
    }
    model_b_prior = c_vec
  ...
  # return(p1)
  return(grid.arrange(grobs = plot_list[1:4],ncol = 2))
